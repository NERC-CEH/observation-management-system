...
Zooming in on the first group of observations marked as outliers, we can see that in the month of May, there were seven, one, and five days where the temperatures exceeded the eccentricity limit.
...
Looking at the density plot we can see that the observation values associated with the deltas that exceed the eccentricity form a distribution with a median value greater than those whos deltas do not exceed the eccentricity.
...
```{r evaluation-ecn-drybulb-delta-2}


#  sd_vals_3 = roll_sd(x=dataset[,get(column)],n=window_size,na.rm=TRUE)
...
# 
# # Holder for the state values
# previous_mean = delta$deltas[1]
# previous_var = 0
# 
# # For each observation in the dataset
# for(k in 1:length(delta$deltas)){
#   curr_vals = rec_teda(delta$deltas[k], previous_mean, previous_var, k)
#   previous_mean = curr_vals[1]
#   previous_var = curr_vals[2]
#   ecc = c(ecc,curr_vals[3])
# 
# }
# 
# dry_temp$delta = c(0,delta$deltas)
# dry_temp$ecc = c(0,ecc)
# dry_temp$norm_ecc = dry_temp$ecc /2
# thresh = 5/(2*(1:17519))
# dry_temp$thresh = c(0,thresh)
# dry_temp$outlier = dry_temp$norm_ecc > dry_temp$thresh
# 
# dry_temp = dry_temp[!is.na(dry_temp$outlier),]
# 
# ggplot(dry_temp) +
#   geom_point(aes(x=doy, y=delta,
#                  colour = outlier)) +
#   facet_grid(year ~ .)
# 
# 
...
Based on what the data around it is doing, but not taking anything else into consideration - dhr likely better by far.

Also really looks like there are signals in the deltas (dry bulb) - auto-correlation.
